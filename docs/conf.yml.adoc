...
Another possible config could look like:
[source,yaml]
----
optim:
  name: ADAM
...
## Freezing a part of network
Freezing a part of network (useful when the model weights are initialized from parent means the weights
remain unmodified during the course of the training.
WKT Optimizer is the one the modifies the parameters, therefore to freeze the weights implies
excluding the weights from optimizer.
