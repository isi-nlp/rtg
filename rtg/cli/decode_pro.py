...
def parse_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=None,
        help='Each token is a valid integer which is an index to embedding,' ' so skip indexifying again',
    )
    parser.add_argument(
        "-bp",
        '--binmt-path',
    ...
    parser.add_argument(
        "-it", '--interactive', action='store_true', help='Open interactive shell with decoder'
    )
    parser.add_argument(
        "-sc",
        '--skip-check',
...
def main():
    ...
    if weights:
        decoder = Decoder.combo_new(exp, model_paths=args.pop('model_path'), weights=weights)
    else:
        decoder = Decoder.new(
