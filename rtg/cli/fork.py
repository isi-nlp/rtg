...
def fork_experiment(
    from_exp: Path, to_exps: Union[Path, List[Path]], conf: bool, vocab: bool, data: bool, code: bool
):
...
def add_on_off_conf(parser, name: str, help, dest=None, default=True):
    ...
    group.add_argument(
        f'--no-{name}', dest=dest, action='store_false', help=f'Negation of --{name}', default=not default
    )


def main():
    p = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description="fork an experiment."
    )
    p.add_argument(
        'from_exp', metavar='EXP_DIR', type=Path, help="From experiment. Should be valid experiment dir"
    )
    p.add_argument(
        'to_exps', metavar='TO_DIR', type=Path, nargs='+', help="To experiment(s). These will be created."
    )
