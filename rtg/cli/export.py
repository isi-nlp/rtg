...
class ExperimentExporter:
    ...
    def export(self, target: Path, name: str = None, ensemble: int = 1, copy_config=True, copy_vocab=True):
        ...
        step_num, train_loss, val_loss = [src_chkpt.get(n, -1) for n in ['step', 'train_loss', 'val_loss']]
        copy_fields = [
            'optim_state',
            'step',
            'train_loss',
        ...
        to_exp.store_model(
            step_num, state, train_score=train_loss, val_score=val_loss, keep=10, prefix=prefix
        )
...
def add_boolean(parser, name, help, dest=None, default=True):
    ...
    group.add_argument(
        f'--no-{name}', dest=dest, action='store_false', default=not default, help=f"See --{name}"
    )
...
def parse_args():
    ...
    p.add_argument('target', metavar='DIR', type=Path, help='Path to destination where the export should be')
    p.add_argument(
        '-en',
        '--ensemble',
        metavar='N',
    ...
    add_boolean(p, 'vocab', dest='copy_vocab', help='Copy vocabulary files (such as sentence piece models)')
