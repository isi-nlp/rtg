...
class RnnLm(SeqDecoder, LanguageModel):
    ...
    def batch_forward(self, batch):
        ...
        for t in range(1, max_seq_len):
            word_probs, last_hidden, _ = self(enc_outs=None, prev_out=prev_out, last_hidden=last_hidden)
...
class RnnLmTrainer(SteppedTrainer):
    ...
    def simple_loss_func(
        self, log_probs, seq_lens, tot_toks=None, max_seq_len=None, train_mode: bool = True
    ) -> float:
    ...
    def run_valid_epoch(self, data_iter):
        ...
        with tqdm(data_iter, total=data_iter.num_batches, unit='batch', dynamic_ncols=True) as data_bar:
            for i, batch in enumerate(data_bar):
                ...
                loss = self.simple_loss_func(
                    outp_log_probs,
                    seq_lens=batch.x_len,
                    tot_toks=batch.x_toks,
                    max_seq_len=batch.max_x_len,
                    train_mode=False,
                )
    ...
    def train(
        ...
        **args,
        ...
        val_data = self.exp.get_mono_data(
            'valid', side, batch_size=batch_size, batch_first=True, sort_dec=True
        )
